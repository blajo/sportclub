...
const Container = styled.div<IConatinerProps>`
  margin-top: ${props => (props.isreadonly === 'true' ? '0' : '8')}px;
  margin-bottom: ${props => (props.isreadonly === 'true' ? '0' : '8')}px;

  &&&&&& .ck-sticky-panel {
    ${props => (props.isreadonly === 'true' ? 'display: none;' : '')}
  }
  &&&&&& .ck-read-only {
    border: 0;
    padding: 0;
...
const CustomCKEditor = observer(({ data, isReadOnly, onReady }: ICustomCKEditor) => {
  ...
  return (
    <Container isreadonly={isReadOnly ? 'true' : 'false'}>
      <CKEditor
